...
- **Internal Service-to-Service Communication (ECS Service Connect):** Services call each other over HTTP using ECS Service Connect. Two CloudMap namespaces are used to avoid a CloudFormation collision:
  ...
  - `sc.internal` — the **ECS Service Connect** mesh namespace. Always pass `ImportedRessources.getServiceConnectNamespace()` to `enableServiceConnect({ namespace: ... })`.
  ...
  - Target service URLs follow the pattern `http://<service-name>.sc.internal:<port>` and must be injected as environment variables in the caller's CDK stack, then read via `@Value` in Spring Boot.
