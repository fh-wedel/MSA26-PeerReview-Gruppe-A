...
const policyFilePath = path.resolve(__dirname, '..', 'verified-permissions', 'workflow-policies.json');
...
const apiStack = new ApiStack(app, 'WorkflowApiStack', {
  env,
  apiName: 'WorkflowServiceAPI',
  description: 'API Gateway for Workflow service',
  targetServiceName: serviceNameContext,
  targetPort: containerPort,
  openApiSpecPath: '../workflow-core/src/main/resources/openapi/workflow.json',
  authorizerConfig: {
...
const serviceStack = new ServiceStack(app, 'WorkflowServiceStack', {
  env,
  serviceName: serviceNameContext,
  imageVersion: imageTag,
  description: 'Workflow Service - provides pluggable review workflow policies.',
