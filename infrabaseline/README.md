...
Damit ein ECS Service Internetzugriff hat (z.B. um AWS APIs aufzurufen), muss er normalerweise in einem öffentlichen Subnetz (mit öffentlicher IPv4) bereitgestellt werden oder über ein NAT Gateway in einem privaten Subnetz (IPv4) laufen. Da IPv4-Adressen und NAT Gateways permanente hohe Kosten verursachen, nutzt dieses Projekt **IPv6-Only Private Subnets**. 
Die ECS Fargate Tasks erhalten eine routbare IPv6-Adresse in einem privaten Subnetz und greifen über ein kostenloses **Egress-Only Internet Gateway (EIGW)** auf das Internet zu. Dadurch wird auf teure NAT Gateways verzichtet, ohne die Sicherheit aufzugeben (eingehender Traffic aus dem Internet wird vom EIGW blockiert).
