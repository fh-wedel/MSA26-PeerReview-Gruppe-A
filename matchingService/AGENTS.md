...
- **DynamoDB Single Table Design:** The service uses the AWS SDK v2 Enhanced Client (`DynamoDbEnhancedClient`). 
  - `MatchRecord` objects use `SUBMISSION#{id}` as PK and `MATCH#{examinerId}` as SK.
  - `SubmissionStatusRecord` objects use `SUBMISSION#{id}` as PK and `STATUS` as SK.
  - Secondary Index (`ExaminerIndex`) allows querying all submissions assigned to an examiner.
- **Amazon Verified Permissions (Cedar):** The infrastructure deploys policies (defined in `infra/verified-permissions/matching-policies.json`). We map groups strictly to their application domain roles (e.g., `Author` and `Reviewer`), avoiding extraneous mapping for dynamic types like `Teacher`.
