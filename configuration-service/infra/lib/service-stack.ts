...
export class ServiceStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: ServiceCreationProps) {
    ...
    // =============================================
    // DynamoDB Table for Configurations
    // =============================================
    const dynamoTableName = 'configuration-service-configs';
    const configsTable = new dynamodb.Table(this, 'ConfigsTable', {
    ...
    configsTable.addGlobalSecondaryIndex({
    ...
    taskDefinition.addContainer('AppContainer', {
      ...
      environment: {
        ...
        'SQS_CACHE_INVALIDATION_QUEUE': 'user-cache-invalidation-queue',
    ...
    if (props.enablePublicIpV4) {
      ...
      ecsSecurityGroup.addIngressRule(ec2.Peer.anyIpv4(), ec2.Port.tcp(8081), 'Inbound HTTP IPv4 on port 8081');
    ...
    // Also allow port 8081
    ecsSecurityGroup.addIngressRule(lambdaSg, ec2.Port.tcp(8081), 'Allow incoming traffic from API Gateway proxy Lambda on port 8081');
    ecsSecurityGroup.addIngressRule(ec2.Peer.anyIpv6(), ec2.Port.tcp(8081), 'Inbound HTTP IPv6 from VPC services on port 8081');
    ...
    configsTable.grantReadWriteData(taskDefinition.taskRole);

    // Grant SQS SendMessage permissions to the matching request queue
    ...
    // Grant SQS SendMessage permissions to the user cache invalidation queue
    taskDefinition.taskRole.addToPrincipalPolicy(
      new iam.PolicyStatement({
        effect: iam.Effect.ALLOW,
        actions: ['sqs:SendMessage', 'sqs:GetQueueAttributes', 'sqs:GetQueueUrl'],
        resources: [
          `arn:aws:sqs:${AWSConstants.AWS_REGION}:${AWSConstants.AWS_ACCOUNT_ID}:user-cache-invalidation-queue`
        ]
      })
    );
